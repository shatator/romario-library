...
if (isset($_POST['insert'])){																				//проверка отправки формы
	...
	if ($_POST['ins_book'] != ""){																			//проверка заполнения поля "Название" 
		$sql = "INSERT INTO `library`.`books` (`book_title`) VALUES ('".$_POST['ins_book']."');";			//добавляем Книгу в таблицу books
			...
			if ($value != ""){																				//если поле автор не пустое
				...
				while($row = mysqli_fetch_assoc($res3)){													//проверяем не совпадает ли имя введённого автора
					if ($value == $row['name']) {															//с именем уже имеющихся в базе авторов
						$sql2 = "INSERT INTO `library`.`book_author` (`book`, `author`) VALUES (".$a.", ".$row['id'].");";
				...
				if ($k == 0) {
					$sql1 = "INSERT INTO `library`.`authors` (`name`) VALUES ('".$value."');";			//иначе добавляем нового автора в таблицу authors
					...
					$sql2 = "INSERT INTO `library`.`book_author` (`book`, `author`) VALUES (".$a.", ".(int)mysqli_fetch_assoc($result2)['id'].");";
